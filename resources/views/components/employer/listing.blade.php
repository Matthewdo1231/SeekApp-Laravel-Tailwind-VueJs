  ...
  <article data-joblisting-id="{{$joblisting -> id}}" id="joblisting-column" class="relative flex gap-6 border-b-[1px] border-gray-400 py-10 px-4 overflow-hidden shadow-md shadow-gray hover:cursor-pointer">
    ...
    <ul data-joblisting-action="{{$joblisting -> id}}" id="listing-action" class="hidden absolute bottom-1 left-10 gap-5 p-2">
        <i data-action-draft="{{$joblisting -> id}}" class="fa-regular fa-file text-gray-500 hover:text-gray-700"><span class="p-2 text-sm font-sans">Move to drafts</span></i>
        <i data-action-active="{{$joblisting -> id}}" class="fa-regular fa-circle-pause text-gray-500 hover:text-gray-700"><span class="p-2 text-sm font-sans">Pause listings</span></i>
        <i data-action-delete="{{$joblisting -> id}}" class="fa-solid fa-trash text-red-500 hover:text-red-600"><span class="p-2 text-sm font-sans">Delete</span></i>
      </ul>
  </article>
